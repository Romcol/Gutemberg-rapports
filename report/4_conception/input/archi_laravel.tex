...
Par exemple si on souhaite créer une page qui va lister tous les articles présents sur notre site, on peut écrire dans le fichier route. Ici on va lier l'url http://www.notresite.extension/articles à la fonction index du contrôleur article.
...
Le contrôleur est le cœur métier de notre application. Il va exécuter les tâches nécessaires à la requête demandée et passer ces informations à la vue.\\
Par exemple, ici on va récupérer tous les articles enregistrés et les passer à la vue "pages.articles". Pour cela il faut tout d'abord communiquer avec le modèle "Article" en lien avec la base de donnée.
...
Le modèle va permettre de définir des objets dans Laravel. Ici par exemple, l'objet Article. Le modèle est en lien avec la base de donnée. On utilise ici Elasticquent(https://github.com/elasticquent/Elasticquent) pour communiquer avec la base de donnée Elasticsearch et Moloquent (https://github.com/jenssegers/laravel-mongodb) pour communiquer avec MongoDB.
...
\paragraph{}
La vue globale contient la structure du site à inclure dans chaque page, c'est ici que nous faire le lien avec Bootstrap et JQuery (utilisé par ce dernier). Nous incluons alors les fichiers CSS et Javascript nécessaires.
